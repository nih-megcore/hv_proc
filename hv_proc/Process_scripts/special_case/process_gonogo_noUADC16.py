...
def main(filename=None, logfile=None, write_mrk_file=True):
    ...
    dframe = append_conditions([response, ppt, logdata])
    dframe.loc[dframe.condition == '1', 'condition'] = 'go'
    dframe.loc[dframe.condition == '2', 'condition'] = 'nogo'
    
    #Crop off overflow to prevent mismatch between logfile and projector
    dframe = crop_logfile_overflow(dframe, channel='UPPT001')
    orig_dframe = copy.deepcopy(dframe)
    
    dframe.loc[dframe.channel=='UPPT001', 'condition']='ppt'
    ...
    for stim in ['triangle', 'diamond', 'pentagon', 'circle', 'square',
        ...
        sub_frame = dframe[dframe['condition'].isin([stim, 'ppt'])]
        stim_idxs = dframe[dframe['condition']==stim].index
        tmp = parse_marks(sub_frame, marker_name='temp', lead_condition='ppt', 
    ...
    dframe.loc[dframe.channel=='UPPT001', 'condition'] = orig_dframe.loc[dframe.channel=='UPPT001', 'condition']
