...
# =============================================================================
# Manually process this 
# The trigger channels were messed up
# Set the correct channels and process
# The responses were not collected on the UADC channels and need to be pulled
# From the logfile
# =============================================================================

optical_chan='UADC006'
# response_chanL = 'UADC007'
# response_chanR = 
filename = '/fast/BIDS_HV_V1/bids/sub-ON48925/ses-01/meg/sub-ON48925_ses-01_task-sternberg_run-01_meg.ds'
logfile = '/fast/BIDS_HV_V1/bids/ON48925_logs/Sternberg-s-ON48925-r-1-.log'
...
def logfile_response(log_file, trial=0, hand=None):
    if hand.lower()=='r':
        keyval='1'
    elif hand.lower()=='l':
        keyval='3'
    else:
        raise('Need to set a handedness response ouptput of l or r')
    dframe=pd.read_csv(log_file, sep='\t', skiprows=2, 
                       names=['onset','DataTxt','Interface'], engine='python')
    dframe['condition']=dframe['Interface'] #tmp_dframe[0]
    dframe['channel'] = 'logfile'
    dframe['trial']=trial
    dframe=dframe[dframe.condition.str[0:8] == 'Keypress'].reset_index(drop=True)
    dframe=dframe[dframe.condition.str[10] == keyval].reset_index(drop=True)
    return dframe[['trial','onset', 'condition', 'channel']]
...
def main(filename, logfile, write_mrk_file=True):
    ...
    response_l = logfile_response(logfile, hand='l')
    response_l.condition = 'response_l'
    
    response_r = logfile_response(logfile, hand='r')
    response_r.condition = 'response_r'
    
    invert_boolean = check_analog_inverted(filename, ch_name=optical_chan)
    projector = threshold_detect(dsname=filename, mark='projector', deadTime=0.5, 
                     derivThresh=0.5, channel=optical_chan, invert=invert_boolean)
